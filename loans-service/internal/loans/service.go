...
type LoanRepository interface {
	HasActiveLoan(ctx context.Context, userID string, bookID string) (bool, error)
	Create(ctx context.Context, loanID string, userID string, bookID string) (*Loan, error)
	ReturnLoan(ctx context.Context, loanID string) (*Loan, error)
	FindActiveByUser(ctx context.Context, userID string) ([]Loan, error)
	FindHistory(ctx context.Context) ([]Loan, error)
}
...
func NewService(repository LoanRepository, libraryClient LibraryBookClient) *Service {
	return &Service{
